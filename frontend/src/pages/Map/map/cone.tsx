...
export default function Cones(props: ConesProps) {
  const { color = new Color(0x8fc2ff), onPointerOver, onPointerOut } = props;
  const texture1 = useTexture(guangquan01);

  return (
    <group position-z={1} renderOrder={5}>
      <Instances limit={50} position-z={0.3}>
...
function Cone(props: ConeProps) {
  const { position, onPointerOver, onPointerOut } = props;
  const ref = useRef<Mesh>(null!);
  let dirRef = useRef<1 | -1>(1);

  useFrame((_, delta) => {
    if (ref.current.position.z >= 0.2) {
      dirRef.current = -1;
    ...
    ref.current.position.z += (dirRef.current * delta) / 4;
  });
